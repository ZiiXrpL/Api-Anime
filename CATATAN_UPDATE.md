# ⚠️ CATATAN: Isi file ini SUDAH TIDAK BERLAKU untuk module Movie

Seluruh isi di bawah ini (source `tv12.lk21official.cc`, lalu pivot ke
`nationalgeographic.com` asli) adalah hasil sesi/pengerjaan LAIN yang
**salah arah** — situs sumber film yang benar (sesuai HTML asli yang sudah
diverifikasi langsung lewat curl) adalah `nationalgeoraphic.com` (perhatikan
ejaannya, memang disengaja beda dari NatGeo asli), bukan National
Geographic sungguhan (yang notabene bukan situs streaming film).

Module Movie saat ini (per update terakhir) 100% ditulis ulang mengikuti
source `nationalgeoraphic.com` (tema WordPress MUVIPRO), dengan endpoint
di `/movie/*` (lihat `src/configs/movieSources.config.ts`,
`src/routes/movie.routes.ts`). Catatan di bawah ini dibiarkan apa adanya
sebagai arsip histori saja, JANGAN dijadikan acuan untuk debugging
selanjutnya.
